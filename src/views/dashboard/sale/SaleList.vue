...
const viewSaleList = (id) => {
  viewStatus.value = true;
  authStore.fetchProtectedApi(`sale/${id}`, {}, 'GET')
    .then((res) => {
...
const searchField = ["sales_invoice_no", "customer_name", "payment_status"];
const searchValue = ref();

const headers = [
  { text: "Sales Invoice No", value: "sales_invoice_no" },
...
const selectedSaleItemId = (id) => {
  selectedSaleItem.value = saleList.value.find(item => item.id === id);
};
  ...
  <div class="card mb-5 mb-xl-8">
    ...
    <div class="card-body py-3">
      ...
      <div v-else>
        ...
        <EasyDataTable sort-by="id" sort-type="desc" buttons-pagination border-cell alternating show-index header-text-direction="center" body-text-direction="center" :headers="headers" :items="saleList" :rows-per-page="10" :search-field="searchField" :search-value="searchValue">
          <template #item-action="{ id }">
            <TheButton data-bs-toggle="modal" data-bs-target=".viewingModel" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" color="success" @click="viewSaleList(id)">
            ...
            <TheButton data-bs-toggle="modal" data-bs-target=".deletingModel" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" color="warning" @click="selectedSaleItemId(id)">
            ...
            <router-link :to="`/dashboard/sale/invoice/${id}`" class="btn btn-icon btn-bg-info btn-active-color-info btn-sm me-1">
              <!--begin::Svg Icon -->
              <span class="svg-icon svg-icon-3">
