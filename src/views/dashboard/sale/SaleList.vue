...
const saleList = ref([]);
const getSaleList = ref(false);

const deletingStatus = ref(false);
const viewStatus = ref(false);
const selectedSaleItem = ref({});
const viewSaleData = ref({});

const getAllSaleList = () => {
  getSaleList.value = false;
  authStore.fetchProtectedApi('sale', {}, 'GET')
    .then((res) => {
      getSaleList.value = true;
      saleList.value = res.data;
    }).catch(err => {
      showAlert('error', err.message || "Failed to fetch sale list");
    }).finally(() => {
      getSaleList.value = true;
    });
};
...
const viewSaleList = (saleItem) => {
  viewStatus.value = true;
  authStore.fetchProtectedApi(`sale/${saleItem.id}`, {}, 'GET')
    .then((res) => {
      viewSaleData.value = res.data;
    }).catch(err => {
      showAlert('error', err.message || "Failed to fetch sale data");
    }).finally(() => {
...
const deleteSaleItem = () => {
  ...
  authStore.fetchProtectedApi(`sale/${selectedSaleItem.value.id}`, {}, 'DELETE')
    .then((res) => {
      $('.deletingModel').modal('hide');
      showAlert('success', res.message || "Sale data deleted successfully");
      getAllSaleList();
    }).catch(err => {
      showAlert('error', err.message || "Failed to delete sale data");
    }).finally(() => {
...
<template>
  <TheBreadcrumb title="Sale List"></TheBreadcrumb>
  ...
  <div class="card mb-5 mb-xl-8">
    ...
    <div class="card-header border-0 pt-5">
      <h3 class="card-title align-items-start flex-column">
        <span class="card-label fw-bolder fs-3 mb-1">Sale List</span>
        <span class="text-muted mt-1 fw-bold fs-7">{{ saleList.length }} Items</span>
      </h3>
      <div class="card-toolbar" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover" title="Click to add">
          <RouterLink to="/dashboard/sale" class="btn btn-primary">
    ...
    <div class="card-body py-3">
      ...
      <div class="text-center" v-if="!getSaleList">Looding...</div>
      <div class="table-responsive" v-else>
        ...
        <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
          ...
          <!--begin::Table body-->
          <tbody>
            <template v-if="saleList.length >= 1">
              <tr v-for="(saleItem, i) in saleList" :key="saleItem.sales_invoice_no">
                <td>{{ i+1 }}</td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.sales_invoice_no }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.customer_name }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.sub_total }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.discount ? saleItem.discount : 0 }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.grand_total }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.payment_status }}</p>
                    </div>
                  </div>
                </td>
                <td>
                  <div class="d-flex align-items-center">
                    <div class="d-flex justify-content-start flex-column">
                      <p class="text-dark fw-bolder text-hover-primary fs-6">{{ saleItem.payment_amount }}</p>
                    </div>
                  </div>
                </td>
                <td class="text-end">
                  <TheButton data-bs-toggle="modal" data-bs-target=".viewingModel" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" color="success" @click="viewSaleList(saleItem)">
                  ...
                  <TheButton data-bs-toggle="modal" data-bs-target=".deletingModel" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" color="warning" @click="selectedSaleItem = saleItem">
                  ...
                  <router-link :to="`/dashboard/sale/invoice/${saleItem.id}`" class="btn btn-icon btn-bg-info btn-active-color-info btn-sm me-1">
                      <!--begin::Svg Icon -->
                      <span class="svg-icon svg-icon-3">
            ...
            <tr v-else>
              <td colspan="50" class="text-center">
                <span class="text-danger">Sale list not found!</span>
              </td>
            </tr>
          </tbody>
          <!--end::Table body-->
  ...
  <TheModel hadding="View Sale Data" action="viewingModel">
    <div class="card text-center">
      <div class="card-header">
        <strong class="my-3 text-center text-info">Invoice No: {{ viewSaleData.sales_invoice_no }}</strong>
      </div>
      <div class="card-body">
          <div class="table-responsive">
              <table class="table table-row-dashed table-row-gray-300 align-middle gs-0 gy-4">
                  ...
                  <tbody>
                    <tr v-for="item in viewSaleData.salesDetails" :key="item.id">
  ...
  <TheModel hadding="Delete Sale List" action="deletingModel">
    <div class="card">
      <div class="card-header d-block text-center">
        <strong>Are you sure you want to delete it?</strong>
        <br>
        <strong class="text-info">Invoice No: {{ selectedSaleItem.sales_invoice_no }}</strong>
      </div>
      <div class="card-body text-center">
        <TheButton data-bs-dismiss="modal">No</TheButton>
        <TheButton color="danger" @click="deleteSaleItem" :loading="deletingStatus">Yes</TheButton>
      </div>
    </div>
  </TheModel>
</template>