...
const selectedMedicineId = (id) => {
  selectedMedicineData.value = medicines.value.find((item) => item.id === id);
};
</script>
    ...
    <div v-else>
      ...
      <EasyDataTable buttons-pagination border-cell alternating show-index header-text-direction="center" body-text-direction="center" :headers="headers" :items="medicines" :rows-per-page="10" :search-field="searchField" :search-value="searchValue">
        ...
        <template #item-price="{ purchases_price, sales_price }">
          <span class="badge badge-primary">Purchases: {{ authStore.defaultSettings.app_currency_symbol }} {{ purchases_price }}</span>
          <span class="badge badge-dark m-1">Sales: {{ authStore.defaultSettings.app_currency_symbol }} {{ sales_price }}</span>
        </template>
        <template #item-action="{ id }">
          ...
          <TheButton data-bs-toggle="modal" data-bs-target=".deletingModel" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm" color="warning" @click="selectedMedicineId(id)">
