    ...
    <table>
      ...
      <tbody>
        <tr v-for="(item, index) in purchaseInvoice.purchasesDetails" :key="index">
          ...
          <td>{{ authStore.defaultSettings.app_currency_symbol }} {{ item.purchases_price }}</td>
          <td>{{ authStore.defaultSettings.app_currency_symbol }} {{ item.purchases_quantity * item.purchases_price }}</td>
        </tr>
      </tbody>
    </table>
  </div>
  <div class="total">
    <p><span class="label">Sub Total:</span> {{ authStore.defaultSettings.app_currency_symbol }} {{ purchaseInvoice.sub_total }}</p>
    <p><span class="label">Discount:</span> {{ authStore.defaultSettings.app_currency_symbol }} {{ purchaseInvoice.discount }}</p>
    <p><span class="label">Grand Total:</span> {{ authStore.defaultSettings.app_currency_symbol }} {{ purchaseInvoice.grand_total }}</p>
    <p><span class="label">Payment Status:</span> {{ purchaseInvoice.payment_status }}</p>
    <p><span class="label">Payment Amount:</span> {{ authStore.defaultSettings.app_currency_symbol }} {{ purchaseInvoice.payment_amount }}</p>
  </div>
</div>
</template>
